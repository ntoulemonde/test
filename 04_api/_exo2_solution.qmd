...
::: {.content-visible when-profile="fr"}
Essayons avec des données plus exhaustives : le fichier brut sur `data.gouv`. Comme on peut le voir dans les métadonnées, on sait qu'on a plus de 1000 écoles dont on peut récupérer des données, mais qu'on en a ici extrait seulement 20. Pour cela, on va utiliser la variable `offset`, qui permet de décaler la réponse envoyée par l'API et de récupérer à partir de la n^ième^ ligne. 
:::

::: {.content-visible when-profile="en"}
Let’s try using more comprehensive data: the raw file on `data.gouv`. As seen in the metadata, we know there are over 1,000 schools for which data can be retrieved, but only 20 have been extracted here. The `offset` field allows us to get data from the n^th^ row.
:::
...
::: {.content-visible when-profile="fr"}
En bouclant sur la variable `offset`, on peut récupérer la totalité des données. Comme le code d'automatisation est assez fastidieux à écrire, le voici :
:::

::: {.content-visible when-profile="en"}
By looping over the `offset`, we can upload the whole dataset. Since the automation code is rather tedious to write, here it is:
:::
...
::: {.content-visible when-profile="fr"}
::: {.callout-note}
On peut aussi utiliser le champ `next` renvoyé dans la requête pour obtenir les données suivantes. Ce champ est construit en incrémentant la variable `offset` selon la même logique.
:::
:::

::: {.content-visible when-profile="en"}
::: {.callout-note}
We can also use the field `next` sent back by the API to directly get the link of the next call. This field is also built on `offset` increment.
:::
:::
