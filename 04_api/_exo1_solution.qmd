...
df_avenue_verdier = pl.DataFrame(output_api_ban).select(['properties']).unnest(['properties'])
```
...
output_q2 = pl.DataFrame(
    output_q2.get("features")).select(['properties']).unnest(['properties'])

output_q2 = gpd.GeoDataFrame(
    output_q2.to_pandas(),
    geometry=gpd.points_from_xy(output_q2.get_column('x').to_pandas(), output_q2.get_column('y').to_pandas()), crs="EPSG:2154"
).to_crs(4326)
output_q2
```
